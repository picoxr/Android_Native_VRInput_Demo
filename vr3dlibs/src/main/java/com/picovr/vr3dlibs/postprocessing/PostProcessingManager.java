...
import com.picovr.vr3dlibs.postprocessing.passes.CopyPass;
import com.picovr.vr3dlibs.postprocessing.passes.EffectPass;
...
public class PostProcessingManager {
    ...
    public PostProcessingManager(@NonNull Renderer renderer,
        ...
        mScene = new Scene(mRenderer, IGraphNode.GRAPH_TYPE.NONE);
    ...
    public void render(@IntRange(from = 0) long ellapsedTime, @FloatRange(from = 0d) double deltaTime) {
        ...
        for (int i = 0; i < mNumPasses; i++) {
            ...
            final boolean depthOrRenderPass = type == IPass.PassType.RENDER || type == IPass.PassType.DEPTH;
            ...
            if (type == IPass.PassType.MASK) {
                maskActive = true;
            } else if (type == IPass.PassType.CLEAR) {
                maskActive = false;
            }
        }
    ...
    private void updatePassesList() {
        ...
        for (int i = 0; i < mComponents.size(); ++i) {
            ...
            } else if (component.getType() == IPostProcessingComponent.PostProcessingComponentType.EFFECT) {
