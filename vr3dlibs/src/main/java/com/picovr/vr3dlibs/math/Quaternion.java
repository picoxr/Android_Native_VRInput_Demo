 ...
 */
package com.picovr.vr3dlibs.math;
...
import com.picovr.vr3dlibs.WorldParameters;
...
public final class Quaternion implements Cloneable {
     ...
     *
     * @param axis  {@link Vector3.Axis} The cardinal axis to set rotation on.
    ...
    public Matrix4 toRotationMatrix(Vector3 v) {
        Matrix4 matrix = new Matrix4();
        toRotationMatrix(matrix,v);
        return matrix;
    }
    /**
    ...
    public Matrix4 toRotationMatrix(@NonNull Matrix4 matrix,Vector3 v) {
        toRotationMatrix(matrix.getDoubleValues(),v);
        return matrix;
    }
    /**
    ...
    public void toRotationMatrix(@NonNull @Size(min = 16) double[] matrix,Vector3 v) {
        final double x2 = x * x;
        final double y2 = y * y;
        final double z2 = z * z;
        final double xy = x * y;
        ...
        matrix[Matrix4.M03] = v.x;
        matrix[Matrix4.M13] = v.y;
        matrix[Matrix4.M23] = v.z;
        matrix[Matrix4.M33] = 1;
    }
    /**
