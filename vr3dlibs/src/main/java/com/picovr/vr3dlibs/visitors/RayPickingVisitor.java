 ...
 */
package com.picovr.vr3dlibs.visitors;

import com.picovr.vr3dlibs.Object3D;
import com.picovr.vr3dlibs.bounds.BoundingBox;
import com.picovr.vr3dlibs.bounds.BoundingSphere;
import com.picovr.vr3dlibs.math.vector.Vector3;
import com.picovr.vr3dlibs.util.Intersector;
...
public class RayPickingVisitor implements INodeVisitor {
	...
	private boolean intersectsWith(BoundingBox bbox) {
		...
		if (raySta.x > boxMin.x && raySta.x < boxMax.x &&
		    ...
		    return true;}
		if ( (getIntersection(raySta.x-boxMin.x, rayEnd.x-boxMin.x, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.X))
		  || (getIntersection(raySta.y-boxMin.y, rayEnd.y-boxMin.y, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.Y))
		  || (getIntersection(raySta.z-boxMin.z, rayEnd.z-boxMin.z, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.Z))
		  || (getIntersection(raySta.x-boxMax.x, rayEnd.x-boxMax.x, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.X))
		  || (getIntersection(raySta.y-boxMax.y, rayEnd.y-boxMax.y, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.Y))
		  || (getIntersection(raySta.z-boxMax.z, rayEnd.z-boxMax.z, raySta, rayEnd) && isInBox(boxMin, boxMax, Vector3.Axis.Z)))
			return true;
	...
	private boolean isInBox(Vector3 boxMin, Vector3 boxMax, Vector3.Axis axis) {
		if ( axis== Vector3.Axis.X && mHitPoint.z > boxMin.z && mHitPoint.z < boxMax.z && mHitPoint.y > boxMin.y && mHitPoint.y < boxMax.y) return true;
		if ( axis== Vector3.Axis.Y && mHitPoint.z > boxMin.z && mHitPoint.z < boxMax.z && mHitPoint.x > boxMin.x && mHitPoint.x < boxMax.x) return true;
		if ( axis== Vector3.Axis.Z && mHitPoint.x > boxMin.x && mHitPoint.x < boxMax.x && mHitPoint.y > boxMin.y && mHitPoint.y < boxMax.y) return true;
		return false;
	}
