 ...
 */
package com.picovr.vr3dlibs.materials.shaders.fragments;
...
public class LightsFragmentShaderFragment extends AShader implements IShaderFragment {
	...
	public void initialize()
	{
		...
		mvEye = (RVec4) addVarying(LightsVertexShaderFragment.LightsShaderVar.V_EYE);
		...
		for (int i = 0; i < mLights.size(); i++)
		{
			...
			muLightPosition[i] = (RVec3) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_LIGHT_POSITION, i);
			muLightPower[i] = (RFloat) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_LIGHT_POWER, i);
			muLightColor[i] = (RVec3) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_LIGHT_COLOR, i);
			
			if(t == ALight.DIRECTIONAL_LIGHT || t == ALight.SPOT_LIGHT)
			{
				muLightDirection[dirLightCount] = (RVec3) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_LIGHT_DIRECTION, dirLightCount);
				dirLightCount++;
			}
			if(t == ALight.SPOT_LIGHT)
			{
				muSpotCutoffAngle[spotLightCount] = (RFloat) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_SPOT_CUTOFF_ANGLE, spotLightCount);
				muSpotFalloff[spotLightCount] = (RFloat) addUniform(LightsVertexShaderFragment.LightsShaderVar.U_SPOT_FALLOFF, spotLightCount);
				spotLightCount++;
			}
			mvAttenuation[i] = (RFloat) addVarying(LightsVertexShaderFragment.LightsShaderVar.V_LIGHT_ATTENUATION, i);
		}
