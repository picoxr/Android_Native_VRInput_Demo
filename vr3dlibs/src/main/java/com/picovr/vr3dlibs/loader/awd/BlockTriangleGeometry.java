...
public class BlockTriangleGeometry extends ABaseObjectBlockParser {
	...
	public void parseBlock(LoaderAWD.AWDLittleEndianDataInputStream dis, LoaderAWD.BlockHeader blockHeader) throws Exception {
		...
		final boolean geoAccuracy = (blockHeader.flags & LoaderAWD.BlockHeader.FLAG_ACCURACY_GEO) ==
				LoaderAWD.BlockHeader.FLAG_ACCURACY_GEO;
		final short geoNr = geoAccuracy ? LoaderAWD.AWDLittleEndianDataInputStream.TYPE_FLOAT64
				: LoaderAWD.AWDLittleEndianDataInputStream.TYPE_FLOAT32;
		...
		for (int parsedSub = 0; parsedSub < mSubGeometryCount; ++parsedSub) {
			...
			if(joints != null && joints.length > 0)
			{
				...
				{
					SkeletalAnimationChildObject3D.BoneVertex bone = new SkeletalAnimationChildObject3D.BoneVertex();
